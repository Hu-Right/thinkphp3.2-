			...
			if($data['type'] == 1){
				...
				if($useremail['currency'] < C("jinkabaodan")){
					...
					if($useremail['zhengsong']==0){
						...
						$this->recordinfo(session('email'),C("jinkabaodan"),"电子币","报单注册会员","减少");
					}else{
						...
						if($xianyou >= C("jinkabaodan")){
							...
							$this->error("您的电子币不足！每次只能用赠送电子的25%,请充值后操作！！");
						}
					}
					$userdata['type'] = 1;
				}
				...
				if($useremail['currency'] < C("zuankabaodan")){
					...
					if($useremail['zhengsong']==0){
						...
						$this->recordinfo(session('email'),C("zuankabaodan"),"电子币","报单注册会员","减少");
					}else{
						$xianyou = $useremail['currency'] - $useremail['zhengsong'] + 2500;
				...
				foreach($quanbu as $v){
					...
					foreach($userquanb as $k=>$s){
						if($s == $usertop[$k]){
							...
							$liangps = M("userpair")->where(array('user'=>$user['email'],'userthis'=>$topuser['email']))->find();
							$liangp =  M("userpair")->where(array('user'=>$v['email'],'userthis'=>$topuser['email']))->find();
							if(!$liangps && !$liangp){
								M("userpair")->add(array('user'=>$user['email'],'userthis'=>$topuser['email']));
								M("userpair")->add(array('user'=>$v['email'],'userthis'=>$topuser['email']));
